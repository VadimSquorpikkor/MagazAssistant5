...
* <b>1.34 - 22/08/21</b>
1. Сделано удаление ордера. Для диалога ордеров добавлена кнопка удалить, добавлен подтверждающий удаление диалог
2. Удалены все лонгКлики
3. Для полей с ценами сделаны двойные + и -: для копеек и рублей
4. Сделана сортировка списка присутствующик по локации + по имени (было только по id имени, новые пользователи появлялись в самом конце списка)
* <b>1.33 - 22/08/21</b>
1. Удален вызов диалогов по клику по заголовкам
2. Сделана панелька для кнопок: сброс покупок, цены, рабочие дни, новый пользователь
3. В ордерах: при клике по юзеру теперь всегда видна кнопка добавления нового ордера 
* <b>1.32 - 21/08/21</b>
1. Добавлен сброс покупок: все ордеры выключаются (но не удаляются), все сотрудники включаются, все дни у сотрудников включаются
* <b>1.31 - 21/08/21</b>
1. Переделан EmployeeDialog
2. Добавлена фун-сть для удаления, редактирования и создания работника (с диалогами и работой с БД)
* <b>1.30 - 19/08/21</b>
1. Продолжение работы с пейджером
* <b>1.29 - 19/08/21</b>
1. Начало работы над пейджером для работников (инфа на весь экран + пролистывание свайпом вправо/влево). При долгом тапе на элеменнте "список заказов" открывается активити с пейджером. Работает, но в пейджере только базовая ф-сть (вывод имени)
* <b>1.28 - 19/08/21</b>
...
3. Для Employee по умолчанию сделано 9 включенных рабочих дней (раньше при переходи на 6 дней у всех работников получался 6-й день НЕ ОТМЕЧЕННЫМ, приходилось у всех вручную включать)
4. Исправлено: для EmployeeDialog теперь отображается имя работника
5. При клике по работнику на странице накладной теперь открывается диалог Работника (EmployeeDialog).
6. Пеключение флага дней для работника теперь сохраняется (сохраняется работник)
7. Для эксперимента сделано 2 варианта открытия диалога EmployeeDialog (ещё не окончено): открытие напрямую из адаптера через переданного менеджера; и открытие через обсервер MutableLiveData (адаптер тыркает mainViewModel, передавая Employee, MainActivity отслеживает этот Mutable и, если не равен null, запускает диалог. Так диалог не пропадает после поворота и не нужно адаптеру передавать менеджер. Не доделано)
* <b>1.26 - 16/04/21</b>
1. Исправлена ошибка объединения локации в unitedLocation, когда в локации нет ни одного работника
2. Начал деллать employeeDialog
* <b>1.25 - 16/04/21</b>
1. Исправлена ошибка открыттия диалога ордера
2. save/load для количества рабочих дней
3. Настройки по умолчанию (данные при создании SQL) для цен и рабочих дней
4. Диалог ввода значения дней
* <b>1.24 - 13/04/21</b>
